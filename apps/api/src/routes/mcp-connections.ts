    ...
    try {
      ...
      if (req.body.refine) {
        ...
        const refineSystemPrompt = format === 'html'
          ? [
              ...
              '  1. NEVER use display:grid or CSS Grid properties (grid-template-columns etc.) — use <table> for multi-column layouts.',
              '  2. display:flex is only safe for single-line nav rows (breadcrumb, tab pills).',
              '  3. Use HTML entities: &rarr; &rsaquo; &uuml; &ouml; &auml; &amp; etc.',
              '  4. Do NOT add background-color or padding to the outermost wrapper div.',
      ...
      const systemPrompt = format === 'html'
        ? [
            ...
            'CRITICAL — Wiki.js HTML sanitizer compatibility rules (violations will break the page):',
            '  1. NEVER use display:grid or display:flex for multi-column card/feature layouts. Use <table cellpadding="0" cellspacing="8" border="0"> instead.',
            '  2. display:flex is ONLY safe for the breadcrumb nav row and the tab pill row (single-line, no wrapping).',
            '  3. NEVER use grid-template-columns, auto-fit, minmax, or any CSS Grid property.',
            '  4. For all feature card grids, object reference tables, related-module rows — always use <table> with inline td styles.',
            '  5. Use HTML entities for special chars: &rarr; &rsaquo; &uuml; &ouml; &auml; &amp; etc.',
            '  6. Do NOT set background-color or padding on the outermost wrapper div.',
            'Style guidelines (proven to work in Wiki.js):',
            ...
            '- Breadcrumb: small <a> tags with color:#8A8985, no underline, › separators via &rsaquo;',
            '- Tab pills: <a> tags with display:inline-block, border-radius:20px, padding:8px 20px, border. Active tab: background:#2D7A4F, color:white. Inactive: background:white, border:1px solid #E0DFDB.',
            '- Hero banner: <div style="background:#2D7A4F;border-radius:12px;padding:32px;"> with white title (32px,600) and muted subtitle',
            '- Feature cards: <table cellpadding="0" cellspacing="8" border="0"><tr><td style="vertical-align:top"> with card divs inside each td',
            '- Card structure: outer div with border:1px solid #E0DFDB, border-radius:12px, overflow:hidden. Header area with background:#EAF5EE, padding:16px 20px. Body with padding:14px 20px 18px.',
            '- Section headings: font-size:18px, font-weight:600, color:#1C1C1A, margin-top:32px',
