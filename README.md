...
- **Bulk streaming translate** — real-time progress bar, per-string confidence scores, cancel mid-run
...
### 🔬 AL Code Analyser
- Drop your AL source folder to extract all objects and their translation coverage
- Per-object breakdown: how many strings are translated vs. missing per XLIFF file
- Click any object → deep-link directly into the Translation editor pre-filtered to that object
- Helps pinpoint exactly which tables, pages, or codeunits still need translation work

### 🧬 Translation Memory (TM Library)
- Auto-populated as you translate: reuse confirmed translations across projects
- Fuzzy-match lookup on every new string (Levenshtein similarity, configurable threshold)
- Full management UI: browse, search, inline edit, bulk delete
- Import from CSV or TMX; export to CSV or TMX
- Stats bar: total entries, languages, usage counts

---

### 📋 Azure DevOps Work Item Management
- View, search, and filter work items (User Stories, Tasks, Bugs, Features, Epics) across ADO projects
- **Backlog view** — hierarchical tree: Epic → Feature → User Story → Task with expand/collapse
  - Iterative expand button: click once = Epics+Features, again = +User Stories, again = +Tasks, again = collapse all
- **Kanban board view** — drag-and-drop cards between state columns (New / Active / Resolved / Closed / …)
...
- **Role-based access** per project:
  | Role | Permissions |
  |------|-------------|
  | **Admin** | Full access: setup, members, invites, all write operations |
  | **Editor** | Translations, work items, documentation — no project setup |
  | **Translator** | Edit and save translations only |
  | **Viewer** | Read-only access |
- **Invite links** — generate shareable invite links (with optional email restriction and 72h TTL); copy link, revoke at any time
- **User Settings** — update display name and change password
- Creator of a project is automatically assigned as Admin
...
│   ├── vscode-extension/  Nexus Translator — VS Code XLIFF editor (AI translate, TM, glossary, Excel review)
│   └── mcp-server/        MCP server exposing translation tools to Copilot Chat
├── packages/
...
### Database setup
```bash
pnpm --filter @nexus/db exec prisma migrate dev --name init
```
...
1. **Sign up** and create your first account
2. **Create a customer & project** in the sidebar
3. **Upload your XLIFF file** in the Translation tab
4. **Set up a glossary** (manually or let AI auto-generate from your XLIFF)
5. **Run AI Translation** on all untranslated strings (streaming, with progress)
6. **Review** the results — use AI Review to catch quality issues
7. **Edit** any remaining strings manually in the inline editor
8. **Download** the updated XLIFF file
...
3. Switch between **List**, **Backlog** (tree), or **Board** (Kanban) views
4. Click a work item to open the detail modal
5. Use **AI Refinement** to improve descriptions / acceptance criteria with context
6. Use **Split / Decompose** to break User Stories into Tasks (with technical specs)
7. Push split items directly to ADO with one click
...
- [x] Multi-user workspace with role-based access per project
- [x] Translation Memory (TM Library) with fuzzy matching, import/export
- [x] AL Code Analyser — translation coverage per BC object
- [x] Sprint/Kanban board view for work items
- [x] Backlog tree view with hierarchical work items
- [x] Bulk AI translation with streaming and confidence scores
